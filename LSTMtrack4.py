...
def generate_data1(sq):
    ...
    for i in range(0, len(sq)-time_step-4):
        ...
        y12.append([sq[i + time_step:i + time_step + 2, :]])
        y123.append([sq[i + time_step:i + time_step + 3, :]])
    ...
    y12s = np.array(y12, dtype=np.float32).squeeze()
    y123s = np.array(y123, dtype=np.float32).squeeze()
    ...
    return xs, ys, y12s, y123s, y2s, y3s, y4s, y5s
...
if __name__=='__main__':
    ...
    y12 = [[] for i in range(10000)]
    y123 = [[] for i in range(10000)]
    ...
    for i in range(0, len(sq_set)):
        if len(sq_set[i]) - time_step - 4 > 1: #caution
            x[j], y[j], y12[j], y123[j], y2[j], y3[j], y4[j], y5[j] = generate_data1(sq_set_diff[i])#这样算完，每一个x[i].shape is [batch_num[i], time_step, input_size]
            j = j + 1

    xs = np.array(x[0])
    ys = np.array(y[0])
    y12s = np.array(y12[0])
    y123s = np.array(y123[0])
    y2s = np.array(y2[0])
    y3s = np.array(y3[0])
    y4s = np.array(y4[0])
    y5s = np.array(y5[0])
    ...
    for i in range(1, j):
        ...
        y12s = np.vstack(((y12s, np.array(y12[i]))))
        y123s = np.vstack(((y123s, np.array(y123[i]))))
    ...
    test_y12 = y12s[train_end:test_end]
    test_y123 = y123s[train_end:test_end]

    # with open(r"D:\轨迹预测\prediction\ARIMA\train_x.pkl", 'wb') as f:
    ...
    # dt = pd.DataFrame(X)
    # dt.to_csv(r"D:\轨迹预测\prediction\GM\test_x.csv")
    # dt = pd.DataFrame(test_y)
    # dt.to_csv(r"D:\轨迹预测\prediction\GM\test_y.csv")
    # Y12 = np.reshape(test_y12, [-1, 5])#周期2
    # dt = pd.DataFrame(Y12)
    # dt.to_csv(r"D:\轨迹预测\prediction\GM\test_y12.csv")
    # Y123 = np.reshape(test_y123, [-1, 5])#周期3
    # dt = pd.DataFrame(Y123)
    # dt.to_csv(r"D:\轨迹预测\prediction\GM\test_y123.csv")
    ...
    for p in range(0, len(optimizer)):
        ...
        x2_org = np.zeros(((test_x.shape[0], test_x.shape[1], test_x.shape[2])), dtype=np.float32)#python默认float64， LSTM需要32
