...
def generate_data1(sq):
    ...
    y2 = []
    sq = np.array(sq)
    for i in range(0, len(sq)-time_step-1):
...
if __name__=='__main__':
    ...
    j = 0
    for i in range(0, len(sq_set)):
        if len(sq_set[i]) - time_step - 1 > 1:
            x[j], y[j], y2[j] = generate_data1(sq_set[i])#这样算完，每一个x[i].shape is [batch_num[i], time_step, input_size]
            j = j + 1
    ...
    for p in range(0, len(optimizer)):
        ...
        train_end2 = int(len(x2_org) * 0.75)
