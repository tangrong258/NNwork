...
my_matrix = np.loadtxt(open(r"C:\Users\tangrong\Desktop\zhil3830.csv", "rb"), delimiter=",", skiprows=0)
flow = np.array(my_matrix, dtype=np.float32)#(38,30)
flow = flow.T
# flow = preprocessing.scale(flow)
flow = flow.reshape(-1,1)
#Sklearn的MinMaxScaler，最简单的归一化
scaler= MinMaxScaler(feature_range=(-1, 1))
flow = scaler.fit_transform(flow).squeeze()

flow = flow.reshape(30,-1)
m = flow.shape[0]
n = flow.shape[1]
...
def maxminnorm(array):
    ...
    data_cols = data_shape[1]#有两种得到维度的方法，rows,cols=np.shape(array)
...
def lstm_model(x, y, is_training):
    ...
    outputs, _ = tf.nn.dynamic_rnn(cell, x, dtype=tf.float32)  # outputs的维度是[BATCH_SIZE,TIME_STEP,cell.outputsize],
    # 保存了每个step中cell的输出值h。
    # output = outputs[:,-1,:]#取最后一步的值，
...
def run_eval(sess, test_x, test_y):
    ...
    # # 字典中的key值即为csv中列名
    # dataframe = pd.DataFrame({'predictions': predictions, 'labels': labels})  # 传入的值不能直接是value，而得是list
    # # #两种写法
    # # columns = ["URL", "predict", "score"]
    # # dt = pd.DataFrame(result_list, columns=columns)
    # # 将DataFrame存储为csv,index表示是否显示行名，default=True
    # dataframe.to_csv(r"C:\Users\tangrong\Desktop\zhiltest.csv", index=False, sep=',')
