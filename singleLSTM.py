...
dt = pd.DataFrame(flytime)
dt.to_csv(r'D:\ZSNJAP01\flight\flightinfo\flytime.csv')
dt = pd.DataFrame(input2)
dt.to_csv(r'D:\ZSNJAP01\flight\flightinfo\input.csv')
...
def lstm_model(x,y,is_training):
    cell = tf.nn.rnn_cell.MultiRNNCell([tf.contrib.rnn.SRUCell(hidden_size)
...
if __name__ == '__main__':
    ...
    for d in index:
        ...
        with open(r'D:\ZSNJAP01\flight\prediction\SVR' + '\\' + howlong + '\\' + 'train_x.pkl', 'wb') as f:
            pickle.dump(train_x, f)
        with open(r'D:\ZSNJAP01\flight\prediction\SVR' + '\\' + howlong + '\\' + 'train_y' + str(d) + '.pkl', 'wb') as f:
            pickle.dump(train_y, f)
        with open(r'D:\ZSNJAP01\flight\prediction\SVR' + '\\' + howlong + '\\' + 'test_x.pkl', 'wb') as f:
            pickle.dump(test_x, f)
        with open(r'D:\ZSNJAP01\flight\prediction\SVR' + '\\'  + howlong + '\\' + 'test_y' + str(d) + '.pkl', 'wb') as f:
            pickle.dump(test_y, f)

        # train_x, train_y7 = generate_data1(input2[0:train_end],flytime[0:train_end])
        # train_y = np.zeros((train_y7.shape[0],1),dtype = np.float32)
        # for i in range(train_y7.shape[0]):
        #     train_y[i,0] = train_y7[i,0]
        #
        # test_x, test_y7 = generate_data1(input2[train_end:test_end],flytime[train_end:test_end])
        # test_y = np.zeros((test_y7.shape[0],1),dtype = np.float32)
        # for i in range(test_y7.shape[0]):
        #     test_y[i,0] = test_y7[i, 0]

        root = r"D:\ZSNJAP01\flight\prediction\SRU" + "\\" + howlong
        sub = "lineavgt"
