...
input2 = input
# input  = np.delete(input,np.s_[2160:2880:1],0)
# flytime = np.delete(flytime,np.s_[2160:2880:1], 0)
# m = m -(2880-2160)
#
# #0-7点的数据不考虑
# k = int(m / 24)
# m = m - 7*k
# input2 = np.zeros((m, n))
# flytime1 = np.zeros((m,flytime.shape[1]))
#
# for i in range(k):
#     input2[17* i:17*(i+1),:] = input[24* i+7:24*(i+1),:]
#     flytime1[17* i:17*(i+1),:] = flytime[24* i+7:24*(i+1),:]
#
#
# flytime = flytime1
...
dt.to_csv(rootfrom + '\\' + 'flytime.csv')
...
hidden_size = 200
layer_num = 2
output_size = ((1,flytime.shape[1]))
train_end = int((31+28+31)*24*(60/sp))
test_end = int(m)
train_step = 5000
    ...
    for i in range(test_step):
        ...
        for k in range(0, ys.shape[1]):
            for l in range(0,ys.shape[0]):
                ...
                b = np.abs(ys[l, k] - pred[l, k]) / (ys[l, k]+0.01)
                mse[l,k] = a
                mae[l,k] = c
                mape[l,k] = b
...
if __name__ == '__main__':
    ...
    sess.run(init)
    train_x, train_y = generate_data1(input2[0:train_end],flytime[0:train_end])
    test_x, test_y = generate_data1(input2[train_end:test_end],flytime[train_end:test_end])
    root = r"D:\ZSNJAP01\flight\prediction\LSTM" + "\\" + howlong
    sub = "lineavgt"
