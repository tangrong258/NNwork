...
def test(sess,x,y):
    ...
    for i in range(test_step):
        ...
        pred_y[i * batch_size:(i + 1) * batch_size, n + test_y.shape[1] * p] = pred[:, 0]
        pred_y[i * batch_size:(i + 1) * batch_size, n+3 + test_y.shape[1] * p] = ys[:, 0]
        # fig = plt.figure()
...
if __name__=='__main__':
    ...
    optimizer = ["Adagrad"]
    pred_y = np.zeros((test_step * batch_size, test_y.shape[1]*2))
